...
4. **Pre-NW off-area filter** (`drop_offarea_boxes`, default on): runs immediately after the music filter, on the same raw pre-fusion nodes, so a marginal box is dropped before it can be silently absorbed into a real line during fusion — the whole reason this must run here rather than as a post-fusion veto like step 5's misdetected-line skip. `_main_text_area()` computes the main chant text area per column from BLLA's own line geometry: lines are grouped into rows by y-overlap (the same rule `fuse_colinear_segments` uses), and a row's *combined* x-coverage — not any single box's width — decides whether it anchors the x- or y-extent, so a real line BLLA fragmented into several small boxes is still recognized. Any line whose own bbox overlaps the resulting area by less than `area_keep_threshold` (default 0.50) of its own area is dropped, recorded on `collection._offarea_filter_dropped`. Like the misdetect veto in step 5 this needs no external annotation, so it protects CLI and `run_chain.py` runs too. See root `README.md` §1e and mothra-text#53.
5. `allocate_lines()` maps each fused line label to a word span using Needleman-Wunsch alignment. Fused lines far too small to hold the text they are offered, whose OCR read nothing, are flagged `misdetected_line_skipped` and assigned no text so they cannot consume a line's words (see 5b). Unlike the music-region filter in step 3 this needs no external annotation — it works from the page's own line geometry — so it also protects CLI and `run_chain.py` runs.
6. `_defuse_manifest()` distributes words back to constituent node labels proportionally by pixel width. This is also why the veto in step 5 operates at the fused level: a tiny segment that y-overlaps a real line is *fused into* it and receives ~0 words from the proportional split, so only a tiny box that forms its own fused line can do damage — the class of box the off-area filter in step 4 is designed to catch earlier instead.
...
The off-area filter (`_main_text_area`, `run_pipeline.py`) is another consumer of the
`[x0, y0, x1, y1]` absolute-corner format — it reads `node.bbox.xmin/ymin/xmax/ymax` directly
off the raw pre-fusion BLLA nodes, never the mothra `[x, y, width, height]` form.
